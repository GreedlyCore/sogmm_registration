/*
    ...
    Uses dlib's trust-region optimization to minimize the alignment cost function.
*/
...
//
//   column_vector x = [tx, ty, tz, u1, u2, u3]
//
//   x[0:2] = tx, ty, tz  → Translation in 3D space
...
int num_func_calls = 0;  // Counter for debugging: tracks how many times cost function is evaluated
int num_hess_calls = 0;  // Counter for debugging: tracks how many times Hessian is computed

// ----------------------------------------------------------------------------------------
// gmm_registration_model: The cost function for GMM alignment
//
// This class defines what dlib tries to minimize. It computes:
//   - Cost value: How well do two GMMs align under a given transformation?
//   - Gradient: Which direction to move the transformation to improve alignment?
//   - Hessian: How curved is the cost surface? (for faster convergence)
//
// Dlib calls operator() repeatedly with different transformations, trying to find
// the one that gives the lowest cost (best alignment).
// ----------------------------------------------------------------------------------------
...
public:
    ...
    // Constructor: Stores the two GMMs we want to align
    // Also pre-computes Et_ (normalization constant) for numerical stability
    gmm_registration_model(const GMM<float, 3>& source_gmm,
                           const GMM<float, 3>& target_gmm):
    ...
    // operator(): Evaluate cost function at transformation x
    //
    // Input: x = [tx, ty, tz, u1, u2, u3] - the 6-parameter transformation
    // Output: Cost value (lower = better alignment)
    //
    // This function is called by dlib's optimizer many times during optimization.
    // It extracts rotation & translation from x, then computes how well the GMMs align.
    float operator() (
        const column_vector& x
    ) const {
      // Extract translation vector from first 3 parameters
      Eigen::Vector3f t_ = Eigen::Vector3f(x(0), x(1), x(2));

      // Extract rotation from last 3 parameters (axis-angle representation)
      Eigen::Vector3f u(x(3), x(4), x(5));
      float unorm = u.norm();  // ||u|| = rotation angle in radians
      num_func_calls++;

      // Convert axis-angle to 3x3 rotation matrix
      // If rotation is nearly zero, use identity matrix for numerical stability
      Eigen::Matrix3f R_;
      if (unorm < 1e-10)
    ...
    // get_derivative_and_hessian(): Compute gradient and Hessian for optimization
    //
...
private:
  ...
  //   - Sum up all pairwise overlaps → total alignment score
  //   - If requested, also compute derivatives for optimization
  //
  // Inputs:
  //   Ri, ti: Current rotation and translation
  //   Nm, Nk: Number of Gaussian components in each GMM
  //   wm, wk: Mixture weights for each component
  //   Lm, Omk: Covariance matrices (flattened into columns)
  //   mu_m, nu_k: Mean positions of Gaussian components
  //   Et: Normalization constant
  //   dRdu, d2Rdu2: Derivatives of rotation matrix w.r.t. axis-angle parameters
  //   compute_grad_hess: Whether to compute derivatives (false for just cost evaluation)
  //
  // Outputs:
  //   score: Alignment cost value
                      ...
                      Eigen::Matrix<float, 6, 6>& H) const
  {
    ...
    #pragma omp parallel if(Nmk > 400)
    {
      ...
      float score_local = 0;
      Eigen::Vector3f dFdt_local = Eigen::Vector3f::Zero();
      Eigen::Matrix3f dFdR_local = Eigen::Matrix3f::Zero();
      Eigen::Matrix<float, 3, 6> Htr_local;
      Eigen::Matrix<float, 9, 3> Delta_local, Gamma_local;
      ...
      // Extract indices for this pair:
      // m_ind = target GMM component index
      // k_ind = source GMM component index
      uint32_t k_ind = mk % Nk;
      uint32_t m_ind = mk / Nk;
      // Extract parameters for this Gaussian pair
      //Берется колонка k_ind из матрицы Omk --> С помощью Eigen::Map интерпретируется как матрица 3×3 (ковариация)
      auto Om = Eigen::Map<const Eigen::Matrix3f>(Omk.col(k_ind).data(), NUM_DIMS, NUM_DIMS); // Source covariance
      auto L  = Eigen::Map<const Eigen::Matrix3f>(Lm.col(m_ind).data(), NUM_DIMS, NUM_DIMS);   // Target covariance
      // Вектор 3×1 из колонки k_ind матрицы nu_k
      auto nu = nu_k.col(k_ind);  // Source mean
      auto mu = mu_m.col(m_ind);  // Target mean
      float w = wm(m_ind) * wk(k_ind);  // Combined weight (product of both component weights)

      // Transform source component mean by current transformation
      Eigen::Vector3f Rnu = Ri * nu;  // Rotate source mean
      Eigen::Vector3f ymk = mu - Rnu - ti;  // target mean - transformed source mean

      // Transform source covariance into target frame
      Eigen::Matrix3f ROmi = Ri * Om;

      // Compute combined covariance Smk = L + R*Om*R'
      // This represents uncertainty in the alignment between components m and k
      Eigen::Matrix3f ROmiR = ROmi * Ri.transpose(); // R * Omk * R'  // R @ Omega_k @ R^T
      Eigen::Matrix3f Smki = L + ROmiR;  // Combined covariance (sum because both GMMs have uncertainty)
      ...
      // Compute Symk = Smk^(-1) * ymk
      // This is used in the Gaussian probability density function
      Eigen::Vector3f Symk = Smk * ymk ;

      // Compute fmk: Gaussian overlap between components m and k
      // This follows the 3D multivariate Gaussian PDF formula:
      // f(y) = (1/√(2π)³√det) * exp(-0.5 * y^T * Σ^(-1) * y)
      const float PI_32 = 1.f/(2*M_PI*sqrt(2*M_PI));  // Normalization constant for 3D Gaussian

      float fmk = w * PI_32 * (1/sqrt(det)) * exp( -0.5f * ymk.transpose() * Symk );
      fmk = fmk / Et;  // Divide by normalization constant for numerical stability
      score_local += fmk;    // Accumulate into THREAD-LOCAL variable (no contention!)

      // Jacobian and Hessian for the rotation matrix wrt axis angle components
      // Evaluate gradient
      // Compute partial derivative wrt t
      dFdt_local.noalias() += fmk * Symk;  // Thread-local accumulation
      ...
      // noalias -- add directly to dFdR_local with no temporary copy
      dFdR_local.noalias() += fmk * (Synut_mk + Sy_ytSROmi - SmkROmi);  // Thread-local

      if (compute_grad_hess)
      {
        // Evaluate Hessian
        // Compute dt^2 terms
        Htr_local.block<3,3>(0,0).noalias() += fmk * (Sy_ytSmk - Smk);  // Thread-local
        ...
        for (int  ix = 0; ix < NUM_DIMS; ix++)
        {
          ...
          Htr_local.block<NUM_DIMS, 1>(0,3+ix).noalias() += dtdr_ix;  // Thread-local
          ...
          Delta_local.block<3, 3>(3*ix, 0) += 0.5 * fmk * (2*Dba + da_qa__db_plus_da_qa__qb + dqadrb_b);  // Thread-local
      ...
      // This is why thread-local accumulators are much faster than per-iteration critical sections
    ...
    // Gradient and Hessian assembly (serial, outside parallel region)
    if (compute_grad_hess)
    {
  ...
  // partial_wrt_u(): Compute derivatives of rotation matrix w.r.t. axis-angle parameters
  //
  // This computes ∂R/∂u1, ∂R/∂u2, ∂R/∂u3 (and optionally second derivatives ∂²R/∂ui∂uj)
  // where R is the 3x3 rotation matrix and u = [u1, u2, u3] is the axis-angle representation.
  //
                            ...
                            std::vector<Eigen::Matrix3f>& J, std::vector<std::vector<Eigen::Matrix3f> >& H) const
  {
    ...
    float t5 = u3*u3;
    float t6 = t3+t4+t5;  // ||u||²
    float t7 = sqrt(t6);  // ||u|| = rotation angle

    // Special case: rotation angle is very small (near identity rotation)
    // Use Taylor series approximation for numerical stability
    // (Full Rodrigues formula becomes numerically unstable near θ=0)
    if (t7 < 1e-10)
    {
      float J_arr[] =
...
// ----------------------------------------------------------------------------------------
// MatcherD2D: Main interface for GMM-to-GMM registration
//
// This is the class you actually use in your code.
// It takes two GMMs and finds the rigid transformation that best aligns them.
// ----------------------------------------------------------------------------------------
class MatcherD2D
{
public:
  // match(): Align source_gmm to target_gmm
  //
  // Inputs:
  //   source_gmm: The GMM to be transformed (moving scan)
  //   target_gmm: The GMM to align to (fixed reference scan)
  //   Tinit: Initial guess for transformation (4x4 matrix with rotation + translation)
  //
  // Outputs:
  //   Tout: Optimized transformation that best aligns the GMMs
  //   return value: Final cost/score (lower is better)
  //
             ...
             float min_delta = 1e-7f)
  {
    ...
    column_vector x(6);
    Eigen::Vector3f t = Tinit.translation();         // Extract translation part
    Eigen::AngleAxisf aa(Tinit.rotation());          // Extract rotation part
    Eigen::Vector3f u = aa.axis() * aa.angle();      // Convert to axis-angle: u = axis * angle
    x = t(0), t(1), t(2), u(0), u(1), u(2);          // Pack into 6-vector
    ...
    Tout =  Eigen::Translation<float,3>( x(0), x(1), x(2))*  // Translation part
            Eigen::AngleAxis<float>(u.norm(),u.normalized());  // Rotation part
