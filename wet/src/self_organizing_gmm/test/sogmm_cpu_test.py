...
class SOGMMCPUTest(unittest.TestCase):
    ...
    @unittest.skip("Skipping test_kernel_comparison")
    def test_kernel_comparison(self):
        """Compare different MeanShift kernels (flat, gaussian, cauchy)"""
        from kernel_comparison import KernelComparison
        ...
        comparison = KernelComparison(bandwidth=self.config.bandwidth)
        results = comparison.run_comparison(
            Y, pcld,
            kernels=['flat', 'gaussian', 'cauchy'],
            visualize=self.plot_show
        )

        # Assert that we got results for all kernels
