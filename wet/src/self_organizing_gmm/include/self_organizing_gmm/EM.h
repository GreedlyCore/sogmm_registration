  ...
  namespace cpu
  {
    ...
    public:
                                            ...
                                            const Container &sogmm)
      {
        ...
        // Parallelize over components (k) - each thread processes one component
        // Use 'if' clause to only parallelize when beneficial (many points)
#pragma omp parallel if(X.rows() > 1000)
        {
