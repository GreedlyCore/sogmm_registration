...
from mean_shift_py import MeanShift as MSf2, KernelType
...
def _kernel_str_to_type(kernel_str):
    kernel_map = {
        'flat': KernelType.FLAT,
        'uniform': KernelType.FLAT,
        'gaussian': KernelType.GAUSSIAN,
        'cauchy': KernelType.CAUCHY,
        'logistic': KernelType.LOGISTIC,
        'epanechnikov': KernelType.EPANECHNIKOV
    }
    return kernel_map.get(kernel_str.lower(), KernelType.FLAT)
...
class SOGMM(object):
    ...
    def __init__(self, bandwidth=0.02, random_state=None, compute='CPU', kernel='flat',
                 save_stats=False, stats_dir=None, stats_file_prefix=None):
        """
        Parameters
        ----------
        ...
        kernel : str, optional (default: 'flat')
            Kernel type for mean-shift. Options: 'flat', 'gaussian', 'cauchy', 'logistic'.
        save_stats : bool, optional (default: False)
            Enable profiling statistics for MeanShift and GMM.
        stats_dir : str, optional (default: None)
            Directory to save profiling statistics.
        stats_file_prefix : str, optional (default: None)
            Prefix for profiling statistics filenames.
        """
    ...
    def fit(self, pcld, mahal_distance=None):
        ...
        if self.save_stats and self.stats_dir and self.stats_file_prefix:
            ms_stats_file = f"ms_stats_{self.stats_file_prefix}.csv"
            kernel_type = _kernel_str_to_type(self.kernel) if isinstance(self.kernel, str) else self.kernel
            ms = MSf2(self.bandwidth, self.save_stats, self.stats_dir, ms_stats_file, kernel_type)
        else:
            ms = MSf2(self.bandwidth, self.kernel)
        ms.fit(ms_data)
    ...
    def gmm_fit(self, pcld, n_components, mahal_distance=None):
        ...
        if self.compute == 'CPU':
            ...
            if self.save_stats and self.stats_dir and self.stats_file_prefix:
                gmm_stats_file = f"gmm_stats_{self.stats_file_prefix}.csv"
