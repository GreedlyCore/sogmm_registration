  ...
  template <typename T, uint32_t N, int K=-1, int S=-1>
    class GMMNBase
    {
      ...
      GMMNBase(const GMMNBase& d)
      {
        ...
        covariances_cholesky_ = d.covariances_cholesky_;
        normal_dist_ = std::normal_distribution<T>(0.0, 1.0);
      }
      ~GMMNBase() { }
      ...
      inline void setCovs(const Eigen::Matrix<T,C,K>& covs)
      {
        ...
        covariances_cholesky_ = Eigen::Matrix<T,C,K>::Zero(C, covs.cols());
        for (int k = 0; k < covs.cols(); ++k)
        {
          Eigen::Matrix<T,N,N> cov = Eigen::Map<Eigen::Matrix<T,N,N>>(
              const_cast<T*>(covs.col(k).data()), N, N);
          Eigen::LLT<Eigen::Matrix<T,N,N>> llt(cov);
          Eigen::Matrix<T,N,N> L = llt.matrixL();
          covariances_cholesky_.col(k) = Eigen::Map<Eigen::Matrix<T,C,1>>(L.data(), C);
        }
      }
	...
	if (covs_.size())
	{
                  ...
                  const T sigma = 3.0) const
      {
        ...
        while (x.size() < n_samples * N)
        {
          ...
          // Rejection sampling: keep points within sigma standard deviations
          if (std::abs(rand_val) < sigma)
            x.push_back(rand_val);
        }

        // Reformat x into N x n_samples matrix
        samples = Eigen::Map<Eigen::Matrix<T,N,-1>>(x.data(), N, n_samples);

        // Count samples per component for batch processing
        std::map<uint32_t, std::vector<uint32_t>> comp_to_indices;
        for (uint32_t i = 0; i < n_samples_comp.size(); ++i)
        {
          comp_to_indices[n_samples_comp[i]].push_back(i);
        }

        // Transform samples using pre-computed Cholesky decomposition
        for (const auto& pair : comp_to_indices)
        {
          uint32_t k = pair.first;
          const std::vector<uint32_t>& indices = pair.second;

          // Get pre-computed Cholesky decomposition
          Eigen::Matrix<T,C,1> chol_vec = covariances_cholesky_.col(k);
          Eigen::Matrix<T,N,N> L = Eigen::Map<Eigen::Matrix<T,N,N>>(chol_vec.data(), N, N);
          Eigen::Matrix<T,N,1> mean = means_.col(k);

          // Transform samples in parallel: x = L * z + mean
          #pragma omp parallel for
          for (size_t idx = 0; idx < indices.size(); ++idx)
          {
            uint32_t i = indices[idx];
            Eigen::Matrix<T,N,1> z = samples.col(i);
            samples.col(i) = L * z + mean;
          }
        }
      }
	...
	Eigen::Matrix<T,K,S> log_gauss_prob = logGaussProb(X);
      ...
      // Pre-computed Cholesky decomposition for efficient sampling
      Eigen::Matrix<T, C, K> covariances_cholesky_;
