...
{
    if(argc!=9) {
	std::cout<<"usage "<<argv[0]<<" x y z r p yaw cloud1.pcd cloud2.pcd\n";
	return -1;
    }
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ> (argv[7], cloud) == -1)
    {
	std::cerr<<"Couldn't read file\n";
	return (-1);
    }
    if (pcl::io::loadPCDFile<pcl::PointXYZ> (argv[8], cloud_offset) == -1)
    {
	std::cerr<<"Couldn't read file\n";
	return (-1);
    }
