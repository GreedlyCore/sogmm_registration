  ...
  namespace gpu
  {
    ...
    public:
      ...
      EM()
      {
        ...
        profiler_ = std::make_shared<TimeProfiler>();
        save_stats_ = false;
      }

      EM(const bool save_stats, const std::string &stats_dir, const std::string &stats_file)
      {
        tol_ = 1e-3;
        reg_covar_ = 1e-6;
        max_iter_ = 100;
        ...
        profiler_ = std::make_shared<TimeProfiler>();
        save_stats_ = save_stats;
        if (save_stats_)
        {
          profiler_->save(stats_dir, stats_file);
        }
      }
      ...
      bool fit(const Tensor &Xt, const Tensor &Respt, Container &sogmm)
      {
        ...
        for (unsigned int n_iter = 0; n_iter <= max_iter_; n_iter++)
        {
          ...
          // E step
          if (save_stats_) profiler_->tic("e_step");
          eStep(Xt, sogmm);
          if (save_stats_) profiler_->toc("e_step");

          // M step
          if (save_stats_) profiler_->tic("m_step");
          mStep(Xt, Log_Resp__.Exp().Reshape({N, K}), sogmm);
          if (save_stats_) profiler_->toc("m_step");
